...
public class Unlock {
    ...
    private static int unlockItem(CommandContext<CommandSourceStack> context) {
        ...
        if (lockedByUUID == null || !lockedByUUID.equals(player.getUniqueId().toString())) {
            ...
            //
            player.sendMessage(ColorFormatHandler.formatComponent(PAPIHandler.parse(player, LocaleHandler.itemWrongLockOwner.replace("%prefix%", LocaleHandler.prefix).replace("%player%", ColorFormatHandler.formatString(lockedOwnerR8)))));
            return 0;
        }
        if (VaultMoneyHandler.isVaultAvailable()) {
